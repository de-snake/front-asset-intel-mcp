...
- Summary: TypeScript MCP server scaffolded with three read-only tools (`list_available_assets`, `get_asset_summary`, `get_asset_research`), static rubric/data layout, APX/APY/PRIME/deSPXA/Saturn seed assets, PT fixed-return overlays, table-facing `agent_display` decisions, scoring-helper `dimensions[].possible_grades` anchors, and a shared normalized `return_context` for both summary JSON and research Markdown. `list_available_assets` returns the static asset roster, accepted lookup strings, and exact summary/research call examples for agents that already know a symbol or address. The return layer covers PT fixed-maturity ROI, non-PT/direct organic or variable ROI, modeled farming value, expected-loss/exit-cost bands, risk-adjusted ROI, and social/X plus quantitative overlays. The latest public rich token report branch (`front-knowledge-base` commit `b954049`) is normalized into returned summaries and reports. A local copy of the asset-investment-diligence research workflow now lives under `docs/asset-investment-diligence/` with a short new-asset run guide.

## Current artifact

- Runtime package: `projects/front-asset-intel-mcp/`
- Research workflow copy: `docs/asset-investment-diligence/` (`RUN-RESEARCH.md` is the short run guide)
- MCP tools:
...
- Summary contract:
  - `list_available_assets` returns `available_asset_count`, per-asset canonical ids/symbols/addresses/aliases/accepted lookup values, and recommended calls for `get_asset_summary` / `get_asset_research` by symbol or address.
  ...
  - The summary and research tools expose the same `return_context` derived from the saved local summary/report package, so research callers and summary callers see the same ROI basis rather than a copied bolt-on block.
...
- `tools/call list_available_assets` returns the 11-asset capability map with accepted lookup values and example summary/research calls.
- `tools/call get_asset_summary` resolves all 11 seed assets: apxUSD, apyUSD, PRIME, deSPXA, USDC, USDat, sUSDat, PT-apxUSD, PT-apyUSD, PT-USDat, and PT-sUSDat.
- `tools/call get_asset_summary` also resolves a chain-prefixed PT market address from the discovery guidance (`ethereum:0x9afe7a057a09cf5da748d952078c9c99938b4329`).
...
- Direct/variable-token summaries return normalized `return_context` with organic ROI, fresh-farming points ROI or no-confirmed-points zero, expected-loss low/base/high bands, social/X context, and risk-adjusted ROI estimates for apxUSD, apyUSD, PRIME, deSPXA, USDC, USDat, and sUSDat.
